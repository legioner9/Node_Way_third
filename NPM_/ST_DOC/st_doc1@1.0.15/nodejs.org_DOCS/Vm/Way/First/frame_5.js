...
fs.readFile(path.join(__dirname, 'code_5.js'), (e, code) => {
    if (e) throw e;
    runCode(code);
});
...
// with inject api throw closure func
    ...
    console.log({exported}); // undefined timers not worked

    console.log({vmx: context.x, vmy: context.y});//timers not worked {vmx: 2, vmy: undefined}
}
